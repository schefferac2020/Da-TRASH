...
class OCIDObject(data.Dataset, datasets.imdb):
    ...
    def __getitem__(self, idx):
        ...
        if cfg.INPUT == 'DEPTH' or cfg.INPUT == 'RGBD':
            ...
            print("THE FILENAME IS: ", pcd_filename)
            #pcloud = pcl.load(pcd_filename).to_array()
            #pcloud[np.isnan(pcloud)] = 0
            cloud = np.asarray(o3d.io.read_point_cloud(pcd_filename).points)
            cloud[np.isnan(cloud)] = 0
            cloud = np.float32(cloud)
            #cloud = cloud.type(torch.cuda.FloatTensor)
            print(cloud.dtype)
            xyz_img = cloud.reshape((self._height, self._width, 3))
            depth_blob = torch.from_numpy(xyz_img).permute(2, 0, 1)
            print(depth_blob.dtype)

            #depth_blob = depth_blob.type(torch.cuda.FloatTensor)
